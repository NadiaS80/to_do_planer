...
class Planer:
    def __init__(self):
        path_file = Path('theplaner.json')
        if path_file.is_file() and path_file.stat().st_size > 0:
            with open('theplaner.json', encoding='utf-8') as f:
                self.planer = json.load(f)
        else:
            self.planer = {}
    ...
    def save(self):
        with open('theplaner.json', 'w', encoding='utf-8') as f:
            json.dump(self.planer, f, ensure_ascii=False, indent=4)
